...
def display_select_mode():
    ...
    回答モードのラジオボタンを表示
    """
    st.sidebar.markdown("**利用目的**") #追加 課題3
...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        ...
        st.info("こんにちは。私は社内文書の情報をもとに回答する生成AIチャットボットです。上記で利用目的を選択し、画面下部のチャット欄からメッセージを送信してください。") #変更 課題3
        st.warning("具体的に入力したほうが期待通りの回答を得やすいです。", icon="⚠️") #追加 課題3

        st.sidebar.divider() #追加 課題3
        # 「社内文書検索」の機能説明
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        if "main_page_number" in message["content"]:
                            st.success(f"{message['content']['main_file_path']} (ページ: {message['content']['main_page_number']})", icon=icon)#課題4
                        else:
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                if "page_number" in sub_choice:
                                    st.info(f"{sub_choice['source']} (ページ: {sub_choice['page_number']})", icon=icon)#課題4
                                else:
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        if "page" in llm_response["context"][0].metadata:
            ...
            st.success(f"{main_file_path} (ページ: {main_page_number})", icon=icon)#課題4
        else:
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            if "page" in document.metadata:
                ...
                # 「ファイルパス」と「ページ番号」
                file_info = f"{file_path}(ページ: {page_number})"#課題4
            else:
                # 「ファイルパス」のみ
